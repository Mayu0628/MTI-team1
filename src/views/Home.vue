<template>
      ...
      <div class="ui segment">
        <form class="ui form" @submit.prevent="getSearchedArticles">
          ...
          <div class="field">
            ...
            <input v-model="search.category" type="text" id="category" name="category" placeholder="カテゴリ" />
          </div>
          ...
          <div class="field">
            ...
            <div class="inline fields">
              <div class="field">
                <input v-model.number="search.start" type="text" id="start" name="start"/>
                <label for="start">から</label>
              </div>

              <div class="field">
                <input v-model.number="search.end" type="text" id="end" name="end"/>
                <label for="end">まで</label>
              </div>
            </div>
          </div>
          <button type="submit" class="ui button huge green fluid">検索</button>
        </form>
      </div>
  ...
  <div class="ui segment">
    <ul class="ui components divided article-list">
      <template v-for="(article, index) in filteredUsers" :key = "index">
        <li class="comment">
          <div class="content">
...
export default {
  ...
  // 計算した結果を変数として利用したいときはここに記述する
    filteredUsers() {
      ...
      e.search.userId?.match(this.userId)
      && e.search.start >= this.search.start
      && e.search.end <= this.search.end
      );
    }
  },
